//FOOTER DATA
export const popularSearchList = [
  { href: "", itemName: "caps & hats," },
  { href: "", itemName: "baggy jeans," },
  { href: "", itemName: "baseball caps," },
  { href: "", itemName: "snapback caps," },
  { href: "", itemName: "dad caps," },
  { href: "", itemName: "athleisure caps," },
  { href: "", itemName: "oversized t-shirts," },
  { href: "", itemName: "regular fit t-shirts," },
  { href: "", itemName: "denims," },
  { href: "", itemName: "half sleeve t-shirts," },
  { href: "", itemName: "oversized t-shirts," },
  { href: "", itemName: "boots," },
  { href: "", itemName: "jeans," },
  { href: "", itemName: "mom jeans" },
];

export const socialMediaIcons = [
  { name: "Facebook", src: "src/assets/Socials/facebook.png" },
  { name: "Instagram", src: "src/assets/Socials/instagram.png" },
  { name: "Pinterest", src: "src/assets/Socials/pinterest.png" },
  { name: "YouTube", src: "src/assets/Socials/youtube.png" },
  { name: "Spotify", src: "src/assets/Socials/spotify.png" },
];

export const coolStuffList = [
  { href: "", itemName: "accessories" },
  { href: "", itemName: "hats & caps" },
  { href: "", itemName: "clothing" },
  { href: "", itemName: "new arrivals" },
  { href: "", itemName: "instagram shop" },
  { href: "", itemName: "streetwear hub" },
  { href: "", itemName: "about us" },
  { href: "", itemName: "blog" },
  { href: "", itemName: "accessories" },
  { href: "", itemName: "offers" },
];

export const boringStuffList = [
  { href: "", itemName: "rewards" },
  { href: "", itemName: "track order" },
  { href: "", itemName: "returns" },
  { href: "", itemName: "support" },
  { href: "", itemName: "contact us" },
  { href: "", itemName: "privacy policy" },
  { href: "", itemName: "about us" },
  { href: "", itemName: "terms & conditions" },
];

//IMAGE CAROUSAL DATA

export const advertisementImages = [
  "src/assets/Ads/baggy-ad.png",
  "src/assets/Ads/boots-ad.png",
  "src/assets/Ads/caps-ad.png",
  "src/assets/Ads/t-shirt-ad.png",
];

//CATEGORY BAR DATA

export const categoryItems = [
  { href: "", itemName: "caps" },
  { href: "", itemName: "t-shirts" },
  { href: "", itemName: "bottoms" },
  { href: "", itemName: "boots" },
];

//PRODUCT CARD DATA

export const bootsCollection = [
  {
    id: 1,
    images: [
      "src/assets/Jeans/baggy_jeans.jpg",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
    title: "BROWN//ANKLE",
    price: "RS.2,500",
    rating: 4,
    size: ["SM/28", "M/32", "L/36"],
  },
  {
    id: 2,
    images: [
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
    title: "BEIGE//UNISEX",
    price: "RS.3,999",
    rating: 3,
    size: ["SM/28", "L/36"],
  },
  {
    id: 3,
    images: [
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
    title: "BROWN//UNISEX",
    price: "RS.1,899",
    rating: 5,
    size: ["SM/28", "M/32"],
  },
  {
    id: 4,
    images: [
      "src/assets/no-logo-black-cap-02_1024x (1).webp",
      "src/assets/no-logo-black-cap-02_1024x (1).webp",
      "src/assets/no-logo-black-cap-02_1024x (1).webp",
      "src/assets/no-logo-black-cap-02_1024x (1).webp",
    ],
    title: "BLACK//WOMEN",
    price: "RS.2,999",
    rating: 4,
    size: ["SM/28"],
  },
  {
    id: 5,
    images: [
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
    title: "CAP//BROWNIE",
    price: "RS.599",
    rating: 3,
    size: ["SM/28"],
  },
  {
    id: 6,
    images: [
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
    title: "T-SHIRT SET//UNISEX",
    price: "RS.2,999",
    rating: 5,
    size: ["M/32", "L/36"],
  },
];

//PRODUCT DETAILED VIEW DATA

export const productDetails = [
  {
    title: "BROWN//ANKLE",
    price: "RS.2,500.00",
    image: [
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
      "src/assets/Jeans/baggy_cargos.webp",
    ],
  },
];
